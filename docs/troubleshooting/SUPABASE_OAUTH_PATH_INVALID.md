...
2. **Supabase generates OAuth URL:**
   ```
   https://wkpgupdorbgcthmjoybe.supabase.co/auth/v1/authorize?
     provider=google&
     redirect_to=nftgo://&
     [other params]
   ```

3. **Browser opens URL** → User signs in with Google
...
6. **App receives deep link** → User is signed in!
